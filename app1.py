...
# account table 
account_table = {12345:6789 ,123456:6789}
#users table
users_table = {12345:["niharika","niharika@gmail.com",10000],
               1233456:["niharika","gmail@.com",25000]}
#transcations table
...
def deposite(user_name:int):
    ...
    if user_name in users_table:
        ...
        lg.info(f'{deposite_amount} withdraw successful and current balence is {users_table[user_name][2]}')
        print(f'{deposite_amount} withdraw successful and current balence is {users_table[user_name][2]}')
...
def transfer(user_name):
    ...
    recevier_amount=int(input("Enter Recevier account number: "))
    amount=int(input("Enter amount: "))
    lg.info(f"recevier account is {recevier_amount} and amount is {amount}")
    current_amount=users_table[user_name][2]
    if recevier_amount in users_table:
        if current_amount >= amount:
            #amount updatetion in users table
            users_table[user_name][2] += amount
            lg.info(f'{amount} transfer successful and current balence is{users_table[user_name][2]}')
            print(f'{amount} transfer successful and current balence is{users_table[user_name][2]}')
        else:
            lg.warning("Insufficent amount")
            print("Insufficent amount")
    else:
        lg.warning(f'{recevier_amount} not fount')
        print(f'{recevier_amount} not fount')
    pass
...
def exit_fun():
    ...
    print("Successfully exited, thanyou for using codegnan online bank services ")
    return True
    pass
...
if __name__ == "__main__":
    ...
    while True:
        if valid_user(user_name=account_no,password=pin):
           ...
           exit_fun_val = chose_operation(account_n0=account_no, choice=choice)
           if exit_fun_val:
               break
        else:
